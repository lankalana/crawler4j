...
import com.github.lankalana.crawler4j.crawler.exceptions.ContentFetchException;
...
import com.github.lankalana.crawler4j.frontier.DocIDServer;
import com.github.lankalana.crawler4j.frontier.Frontier;
import com.github.lankalana.crawler4j.parser.HtmlParseData;
import com.github.lankalana.crawler4j.parser.NotAllowedContentException;
import com.github.lankalana.crawler4j.parser.ParseData;
...
public class WebCrawler implements Runnable {
	...
	/**
	 * The controller instance that has created this crawler thread. This reference to the controller can be used for
	 * getting configurations of the current crawl or adding new seeds during runtime.
	 */
	...
	/**
	 * The RobotstxtServer instance that is used by this crawler instance to determine whether the crawler is allowed to
	 * crawl the content of each page.
	 */
	...
	/**
	 * Is the current crawler instance waiting for new URLs? This field is mainly used by the controller to detect
	 * whether all of the crawler instances are waiting for new URLs and therefore there is no more work and crawling
	 * can be stopped.
	 */
	...
	/**
	 * This function is called just before starting the crawl by this crawler instance. It can be used for setting up
	 * the data structures or initializations needed by this crawler instance.
	 */
	public void onStart() {
	...
	/**
	 * This function is called just before the termination of the current crawler instance. It can be used for
	 * persisting in-memory data or other finalization tasks.
	 */
	...
	/**
	 * This function is called once the header of a page is fetched. It can be overridden by sub-classes to perform
	 * custom logic for different status codes. For example, 404 pages can be logged, etc.
	 *
	...
	/**
	 * This function is called before processing of the page's URL It can be overridden by subclasses for tweaking of
	 * the url before processing it. For example, http://abc.com/def?a=123 - http://abc.com/def
	 *
	...
	/**
	 * The CrawlController instance that has created this crawler instance will call this function just before
	 * terminating this crawler thread. Classes that extend WebCrawler can override this function to pass their local
	 * data to their controller. The controller then puts these local data in a List that can then be used for
	 * processing the local data of crawlers (if needed).
	 *
	...
	/**
	 * Classes that extends WebCrawler should overwrite this function to tell the crawler whether the given url should
	 * be crawled or not. The following default implementation indicates that all urls should be included in the crawl
	...
	/**
	 * Determine whether links found at the given URL should be added to the queue for crawling. By default this method
	 * returns true always, but classes that extend WebCrawler can override it in order to implement particular policies
	 * about which pages should be mined for outgoing links and which should not.
	 *
	 * <p>If links from the URL are not being followed, then we are not operating as a web crawler and need not check
	 * robots.txt before fetching the single URL. (see definition at http://www.robotstxt.org/faq/what.html). Thus URLs
	 * that return false from this method will not be subject to robots.txt filtering.
	 *
	...
	/**
	 * Classes that extends WebCrawler should overwrite this function to process the content of the fetched and parsed
	 * page.
	 *
	...
	private void processPage(WebURL curURL) throws IOException, InterruptedException, ParseException {
		...
		try {
			...
			handlePageStatusCode(
					curURL, statusCode, EnglishReasonPhraseCatalog.INSTANCE.getReason(statusCode, Locale.ENGLISH));
			...
			if (statusCode < 200 || statusCode > 299) { // Not 2XX: 2XX status codes indicate success
				if (statusCode == HttpStatus.SC_MOVED_PERMANENTLY
					...
					if (myController.getConfig().isFollowRedirects()) {
						...
						if (shouldVisit(page, webURL)) {
							if (!shouldFollowLinksIn(webURL) || robotstxtServer.allows(webURL)) {
								...
								logger.debug(
										"Not visiting: {} as per the server's \"robots.txt\" policy", webURL.getURL());
							}
						} else {
					...
					String description = EnglishReasonPhraseCatalog.INSTANCE.getReason(
							fetchResult.getStatusCode(), Locale.ENGLISH); // Finds
						...
						if (newdocid > 0) {
							...
							if ((maxCrawlDepth == -1) || (curURL.getDepth() < maxCrawlDepth)) {
								if (shouldVisit(page, webURL)) {
									...
									logger.debug(
											"Not visiting: {} as per your \"shouldVisit\" policy", webURL.getURL());
								}
							}
						}
					}
			...
			logger.debug(
					"Skipping: {} as it contains binary content which you configured not to crawl", curURL.getURL());
