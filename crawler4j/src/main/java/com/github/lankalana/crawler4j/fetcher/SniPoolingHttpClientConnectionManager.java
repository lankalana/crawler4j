package com.github.lankalana.crawler4j.fetcher;
...
/**
 * Class to work around the exception thrown by the SSL subsystem when the server is incorrectly configured for SNI. In
 * this case, it may return a warning: "handshake alert: unrecognized_name". Browsers usually ignore this warning, while
 * Java SSL throws an exception.
 *
 * <p>This class extends the PoolingHttpClientConnectionManager to catch this exception and retry without the configured
 * hostname, effectively disabling the SNI for this host.
 *
 * <p>Based on the code provided by Ivan Shcheklein, available at:
 *
 * <p>http://stackoverflow.com/questions/7615645/ssl-handshake-alert-unrecognized-name-error-since
...
public class SniPoolingHttpClientConnectionManager extends PoolingHttpClientConnectionManager {
	...
	@Override
	public void connect(
			final HttpClientConnection conn, final HttpRoute route, final int connectTimeout, final HttpContext context)
			throws IOException {
		try {
