...
/**
 * The controller that manages a crawling session. This class creates the crawler threads and monitors their progress.
 *
...
public class CrawlController {
	...
	/**
	 * The 'customData' object can be used for passing custom crawl-related configurations to different components of
	 * the crawler.
	 */
	...
	/**
	 * Is the crawling session set to 'shutdown'. Crawler threads monitor this flag and when it is set they will no
	 * longer process new pages.
	 */
	...
	/**
	 * Start the crawling session and wait for it to finish. This method utilizes default crawler factory that creates
	 * new crawler using Java reflection
	 *
	...
	/**
	 * Start the crawling session and wait for it to finish. This method depends on a single instance of a crawler. Only
	 * that instance will be used for crawling.
	 *
	...
	/**
	 * Start the crawling session and return immediately. This method utilizes default crawler factory that creates new
	 * crawler using Java reflection
	 *
	 * @param clazz the class that implements the logic for crawler threads
	 * @param numberOfCrawlers the number of concurrent threads that will be contributing in this crawling session.
	 * @param <T> Your class extending WebCrawler
	...
	/**
	 * Once the crawling session finishes the controller collects the local data of the crawler threads and stores them
	 * in a List. This function returns the reference to this list.
	 *
	...
	/**
	 * Adds a new seed URL. A seed URL is a URL that is fetched by the crawler to extract new URLs in it and follow them
	 * for crawling.
	 *
	...
	/**
	 * Adds a new seed URL. A seed URL is a URL that is fetched by the crawler to extract new URLs in it and follow them
	 * for crawling. You can also specify a specific document id to be assigned to this seed URL. This document id needs
	 * to be unique. Also, note that if you add three seeds with document ids 1,2, and 7. Then the next URL that is
	 * found during the crawl will get a doc id of 8. Also you need to ensure to add seeds in increasing order of
	 * document ids.
	 *
	 * <p>Specifying doc ids is mainly useful when you have had a previous crawl and have stored the results and want to
	 * start a new crawl with seeds which get the same document ids as the previous crawl.
	 *
	 * @param pageUrl the URL of the seed
	...
	/**
	 * This function can called to assign a specific document id to a url. This feature is useful when you have had a
	 * previous crawl and have stored the Urls and their associated document ids and want to have a new crawl which is
	 * aware of the previously seen Urls and won't re-crawl them.
	 *
	 * <p>Note that if you add three seen Urls with document ids 1,2, and 7. Then the next URL that is found during the
	 * crawl will get a doc id of 8. Also you need to ensure to add seen Urls in increasing order of document ids.
	 *
	...
	/**
	 * @deprecated implements a factory {@link WebCrawlerFactory} and inject your cutom data as shown <a
	 *     href="https://github.com/yasserg/crawler4j#using-a-factory">here</a> .
	 */
	@Deprecated
	...
	/**
	 * Set the current crawling session set to 'shutdown'. Crawler threads monitor the shutdown flag and when it is set
